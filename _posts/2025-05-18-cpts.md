...
|Command|Description|
|---|---|
{% raw %}|`#!/bin/bash for x in {{A..Z},{0..9}}{{A..Z},{0..9}}{{A..Z},{0..9}}{{A..Z},{0..9}} do echo $x; done`|Bash script used to generate `16,079,616` possible username combinations from a Linux-based host.|{% endraw %}
...
|`rpcclient -U "" -N 172.16.5.5 -c "enumdomusers"`|Enumerates user accounts in a target Windows domain using rpcclient from a Linux host.|
